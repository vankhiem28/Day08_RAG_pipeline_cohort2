...
# OpenAI chat model dùng cho bước generation. Có thể override bằng OPENAI_MODEL trong .env.
...
def _source_label(chunk: dict, index: int) -> str:
    ...
    return source.replace(".md", "").replace(".pdf", "").replace(".docx", "")
...
def _extractive_answer(query: str, chunks: list[dict]) -> str:
    ...
    for index, chunk in enumerate(chunks, 1):
        ...
        parts = re.split(r"(?<=[.!?])\s+|\n+", content)
        snippet = next((p.strip() for p in parts if len(p.strip()) > 40), content[:260])
        label = _source_label(chunk, index)
        sentences.append(f"- {snippet[:320]} [{label}]")
        if len(sentences) >= 3:
            break

    intro = "Dựa trên các nguồn đã truy xuất, câu trả lời có thể tóm tắt như sau:"
    return intro + "\n" + "\n".join(sentences)
